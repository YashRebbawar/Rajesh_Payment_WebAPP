...
ALLOWED_EMAIL_DOMAINS = os.getenv('ALLOWED_EMAIL_DOMAINS', 'gmail.com').split(',')
ALLOWED_EMAIL_DOMAINS = [domain.strip().lower() for domain in ALLOWED_EMAIL_DOMAINS]
...
def api_register():
    ...
    if not validate_email_domain(email):
        ...
        return jsonify({'success': False, 'message': f'Email domain not allowed. Allowed domains: {', '.join(ALLOWED_EMAIL_DOMAINS)}'}), 400
...
def api_signin():
    ...
    if not validate_email_domain(email):
        logger.warning(f"Sign in attempt with invalid domain: {email}")
        return jsonify({'success': False, 'message': f'Email domain not allowed. Allowed domains: {', '.join(ALLOWED_EMAIL_DOMAINS)}'}), 400
    ...
    logger.warning(f"Failed sign in attempt for: {email}")
...
def google_callback():
    try:
        ...
        if not validate_email_domain(user_info['email']):
            logger.warning(f"Google OAuth attempt with invalid domain: {user_info['email']}")
            return redirect(url_for('signin'))
